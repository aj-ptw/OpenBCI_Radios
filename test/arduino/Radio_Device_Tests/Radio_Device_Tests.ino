...
void go() {
    ...
    testBufferStreamSwapHead();
    testBufferStreamSwapTail();
...
void testBufferStreamSwapHead() {
    test.describe("bufferStreamSwapHead");

    test.it("should move the head to the buffer 1 when it's empty");
    testBufferStreamCleanUp();
    radio.streamPacketBuffer->bytesIn = 32;
    radio.streamPacketBuffer->state = radio.STREAM_STATE_READY;
    test.assertBoolean(radio.bufferStreamSwapHead(),true,"should find a new buffer",__LINE__);
    test.assertEqualInt(radio.streamPacketBufferHead,1,"should move the head to postion 1",__LINE__);

    test.it("should move the head to the buffer 2 when it's empty");
    testBufferStreamCleanUp();
    radio.streamPacketBuffer->bytesIn = 32;
    radio.streamPacketBuffer->state = radio.STREAM_STATE_READY;
    (radio.streamPacketBuffer + 1)->bytesIn = 32;
    (radio.streamPacketBuffer + 1)->state = radio.STREAM_STATE_READY;
    radio.streamPacketBufferHead = 1;
    test.assertBoolean(radio.bufferStreamSwapHead(),true,"should find a new buffer",__LINE__);
    test.assertEqualInt(radio.streamPacketBufferHead,2,"should move the head to postion 2",__LINE__);

    test.it("should move the head to buffer 0 when it's empty");
    testBufferStreamCleanUp();
    (radio.streamPacketBuffer + 1)->bytesIn = 32;
    (radio.streamPacketBuffer + 1)->state = radio.STREAM_STATE_READY;
    (radio.streamPacketBuffer + 2)->bytesIn = 32;
    (radio.streamPacketBuffer + 2)->state = radio.STREAM_STATE_READY;
    radio.streamPacketBufferHead = 2;
    test.assertBoolean(radio.bufferStreamSwapHead(),true,"should find a new buffer",__LINE__);
    test.assertEqualInt(radio.streamPacketBufferHead,0,"should move the head to postion 0",__LINE__);

    test.it("should not move the head if the head is still storing");
    testBufferStreamCleanUp();
    radio.streamPacketBuffer->bytesIn = 5;
    radio.streamPacketBuffer->state = radio.STREAM_STATE_STORING;
    radio.streamPacketBufferHead = 0;
    test.assertBoolean(radio.bufferStreamSwapHead(),false,"should not move to a new buffer",__LINE__);
    test.assertEqualInt(radio.streamPacketBufferHead,0,"should not move the head to postion 1",__LINE__);

    test.it("should not move the head if the head is in tail position");
    testBufferStreamCleanUp();
    radio.streamPacketBuffer->bytesIn = 32;
    ...
    test.it("should find the third buffer if the first and the second are ready");
    testBufferStreamCleanUp();
    radio.streamPacketBuffer->bytesIn = 32;
    radio.streamPacketBuffer->state = radio.STREAM_STATE_READY;
    (radio.streamPacketBuffer + 1)->bytesIn = 32;
    (radio.streamPacketBuffer + 1)->state = radio.STREAM_STATE_READY;
    radio.streamPacketBufferHead = 0;
    test.assertBoolean(radio.bufferStreamSwapHead(),true,"should move to a new buffer",__LINE__);
    test.assertEqualInt(radio.streamPacketBufferHead,2,"should not move the head to postion 1",__LINE__);

    test.it("should find the second buffer if the first and the thrid are ready");
    testBufferStreamCleanUp();
    radio.streamPacketBuffer->bytesIn = 32;
    radio.streamPacketBuffer->state = radio.STREAM_STATE_READY;
    (radio.streamPacketBuffer + 2)->bytesIn = 32;
...
void testBufferStreamSwapTail() {
    test.describe("bufferStreamSwapTail");

    test.it("should not swap the tail");
    testBufferStreamCleanUp();
    radio.streamPacketBufferHead = 0;
    radio.streamPacketBufferTail = 0;
    test.assertBoolean(radio.bufferStreamSwapTail(),false,"should not move the tail",__LINE__);
    test.assertEqualInt(radio.streamPacketBufferTail,0,"should not move the tail",__LINE__);

    test.it("should move the the tail to the head up 1");
    testBufferStreamCleanUp();
    radio.streamPacketBufferHead = 1;
    radio.streamPacketBufferTail = 0;
    test.assertBoolean(radio.bufferStreamSwapTail(),true,"should move the tail",__LINE__);
    test.assertEqualInt(radio.streamPacketBufferTail,radio.streamPacketBufferHead,"should move the tail to the head",__LINE__);

    test.it("should move the the tail towards the head");
    testBufferStreamCleanUp();
    radio.streamPacketBufferHead = 2;
    radio.streamPacketBufferTail = 0;
    test.assertBoolean(radio.bufferStreamSwapTail(),true,"should move the tail",__LINE__);
    test.assertEqualInt(radio.streamPacketBufferTail,1,"should move the tail one up towards the head",__LINE__);

    test.it("should move the the tail towards the head");
    testBufferStreamCleanUp();
    radio.streamPacketBufferHead = 0;
    radio.streamPacketBufferTail = 1;
    test.assertBoolean(radio.bufferStreamSwapTail(),true,"should move the tail",__LINE__);
    test.assertEqualInt(radio.streamPacketBufferTail,2,"should move the tail towards the end",__LINE__);

    test.it("should move the the tail towards the head");
    testBufferStreamCleanUp();
    radio.streamPacketBufferHead = 0;
    radio.streamPacketBufferTail = 2;
    test.assertBoolean(radio.bufferStreamSwapTail(),true,"should move the tail",__LINE__);
    test.assertEqualInt(radio.streamPacketBufferTail,0,"should move the tail to postion 1",__LINE__);
